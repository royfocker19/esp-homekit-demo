...
void button_intr_callback(uint8_t gpio) {
    ...
    if (gpio_read(button->gpio_num) == button->pressed_value) {
        ...
        if ((now - button->last_press_time) * portTICK_PERIOD_MS > button->long_press_time) {
            ...
            if ((now - button->last_press_time) * portTICK_PERIOD_MS > 2000 && (now - button->last_press_time) * portTICK_PERIOD_MS < 4000) {
                button->callback(button->gpio_num, button_event_medium2_press);
           } else {
            if ((now - button->last_press_time) * portTICK_PERIOD_MS > 4000 && (now - button->last_press_time) * portTICK_PERIOD_MS < 7000) {
                button->callback(button->gpio_num, button_event_medium3_press);
           } else {
            if ((now - button->last_press_time) * portTICK_PERIOD_MS > 7000 && (now - button->last_press_time) * portTICK_PERIOD_MS < 12000) {
                button->callback(button->gpio_num, button_event_medium4_press);
            } else {
